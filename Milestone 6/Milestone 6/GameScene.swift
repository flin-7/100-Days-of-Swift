...
//  Milestone 6
//
//  Created by Felix Lin on 11/24/19.
...
class GameScene: SKScene {
    ...
    var bulletsClip = 3 {
        didSet {
            bulletsSprite.texture = bulletTextures[bulletsClip]
        }
    }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if isGameOver {
            ...
            if bulletsClip > 0 {
                ...
                shot(at: location)
            } else if bulletsClip <= 0 && tappedNode.contains(where: { $0.name == "reload" }) {
    ...
    func createOverlay() {
        ...
        reloadLabel.position = CGPoint(x: 300, y: 50)
    ...
    func shot(at location: CGPoint) {
        let hideNodes = nodes(at: location).filter { $0.name == "target" }
        
        guard let hitNode = hideNodes.first else { return }
